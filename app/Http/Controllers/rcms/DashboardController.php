...
use App\Models\{Division,Deviation, Document, extension_new, ExternalAudit, Incident, Meeting, OOT, RecallProcecss, SupplierAudit};
...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // });
        $datasQuery->where(function ($query) use ($notificationTable, $loggedInUserId) {
            $query->whereExists(function ($query) use ($notificationTable, $loggedInUserId) {
                $query->select(DB::raw(1))
                    ->from($notificationTable)
                    ->whereRaw("$notificationTable.record_id = c_c_s.id")
                    ->where("$notificationTable.to_id", $loggedInUserId)
            ...
            ->orWhereNotExists(function ($query) use ($notificationTable, $loggedInUserId) {
                $query->select(DB::raw(1))
                    ->from($notificationTable)
                    ->whereRaw("$notificationTable.record_id = c_c_s.id")
                    ->where("$notificationTable.to_id", $loggedInUserId)
        ...
        $requestprocessQuery = RecallProcecss::query();
        ...
        if ($filterType !== 'all' && $filterType !== '' && $filterType !== 'custom') {
            ...
            $requestprocessQuery->whereNotIn('status', ['Closed - Done']);
        ...
        switch ($filterType) {
            case 'weekly':
                ...
                $requestprocessQuery->whereBetween('created_at', [$startOfPreviousWeek, $endOfPreviousWeek]);
            ...
            case 'fortnight':
                ...
                $requestprocessQuery->whereBetween('created_at', [$startOfPreviousfortnight, $endOfPreviousfortnight]);
            ...
            case 'quarterly':
                ...
                $requestprocessQuery->whereBetween('created_at', [$startOfPreviousforquarterly, $endOfPreviousfortquarterly]);
            ...
            case 'halfyearly':
                ...
                $requestprocessQuery->whereBetween('created_at', [$startOfPrevioushalfyearly, $endOfPrevioushalfyearly]);
            ...
            case 'monthly':
                ...
                $requestprocessQuery->whereBetween('created_at', [$startOfMonth, $endOfMonth]);
            ...
            case 'annually':
                ...
                $requestprocessQuery->whereBetween('created_at', [$startOfYear, $endOfYear]);
            ...
            case 'custom':
                if ($startDate && $endDate) {
                    ...
                    $requestprocessQuery->whereBetween('created_at', [$start, $end]);
        ...
        $requestprocess = $requestprocessQuery->orderByDesc('id')->get();
        ...
        foreach ($requestprocess as $data) {
            $data->create = Carbon::parse($data->created_at)->format('d-M-Y h:i A');
        ...
        foreach ($equipment as $data) {
            ...
            array_push($table, [
                ...
                "parent" => $data->parent_record ? $data->parent_record : "-",
                "record" => $data->record,
                "type" => "Equipment Lifecycle Management",
                "parent_id" => $data->parent_id,
                "parent_type" => $data->parent_type,
                "division_id" => $data->division_id,
                "short_description" => $data->short_description ? $data->short_description : "-",
    ...
    public function ccView($id, $type)
    {
        ...
        elseif ($type == "Equipment Lifecycle Management") {
        ...
        elseif ($type == "Recall Process") {
            $data = RecallProcecss::find($id);
            $single = "recall-process-single-pdf/" . $data->id;
            $family = "recall-process-family-pdf".$data->id;
            $audit = "recall-process-audit-report/" . $data->id;
            $division = QMSDivision::find($data->division_id);
            $division_name = $division->name;
        }
...
// }
    ...
    public function qmsRecordAnalytics(Request $request)
    {
        ...
        $fieldMappings = [
            'Change Control' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'HOD Assessment' => ['start' => 'submit_on', 'end' => 'hod_review_on', 'responsible' => 'hod_review_by'],
                'QA/CQA Initial Assessment' => ['start' => 'hod_review_on', 'end' => 'QA_initial_review_on', 'responsible' => 'QA_initial_review_by'],
                'CFT Assessment' => ['start' => 'QA_initial_review_on', 'end' => 'cft_review_on', 'responsible' => 'cft_review_by'],
                'QA/CQA Final Review' => ['start' => 'cft_review_on', 'end' => 'QA_final_review_on', 'responsible' => 'QA_final_review_by'],
                'Pending RA Approval' => ['start' => 'QA_final_review_on', 'end' => 'pending_RA_review_on', 'responsible' => 'pending_RA_review_by'],
                'QA/CQA Head/Manager Designee Approval' => ['start' => 'pending_RA_review_on', 'end' => 'QA_head_approval_on', 'responsible' => 'QA_head_approval_by'],
                'Pending Initiator Update' => ['start' => 'QA_head_approval_on', 'end' => 'initiator_update_complete_on', 'responsible' => 'initiator_update_complete_by'],
                'HOD Final Review' => ['start' => 'initiator_update_complete_on', 'end' => 'HOD_finalReview_complete_on', 'responsible' => 'HOD_finalReview_complete_by'],
                'Implementation Verification by QA/CQA' => ['start' => 'initiator_update_complete_on', 'end' => 'closure_approved_on', 'responsible' => 'closure_approved_by'],
                'QA/CQA Closure Approval' => ['start' => 'closure_approved_on', 'end' => 'closure_approved_on', 'responsible' => 'closure_approved_by'],
            ],

            'CAPA' => [
                'Opened' => ['start' => 'created_at', 'end' => 'plan_proposed_on', 'responsible' => 'plan_proposed_by'],
                'HOD Review' => ['start' => 'plan_proposed_on', 'end' => 'hod_review_completed_on', 'responsible' => 'hod_review_completed_by'],
                'QA/CQA Review' => ['start' => 'hod_review_completed_on', 'end' => 'qa_review_completed_on', 'responsible' => 'qa_review_completed_by'],
                'QA/CQA Approval' => ['start' => 'qa_review_completed_on', 'end' => 'approved_on', 'responsible' => 'approved_by'],
                'CAPA In progress' => ['start' => 'approved_on', 'end' => 'completed_on', 'responsible' => 'completed_by'],
                'HOD Final Review' => ['start' => 'completed_on', 'end' => 'hod_final_review_completed_on', 'responsible' => 'hod_final_review_completed_by'],
                'QA/CQA Closure Review' => ['start' => 'hod_final_review_completed_on', 'end' => 'qa_closure_review_completed_on', 'responsible' => 'qa_closure_review_completed_by'],
                'QAH/CQAH Approval' => ['start' => 'qa_closure_review_completed_on', 'end' => 'qah_approval_completed_on', 'responsible' => 'qah_approval_completed_by'],
                'Closed - Done' => ['start' => 'qah_approval_completed_on', 'end' => 'qah_approval_completed_on', 'responsible' => 'qah_approval_completed_by'],
            ],

            'Deviation' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'HOD Review' => ['start' => 'submit_on', 'end' => 'hod_review_completed_on', 'responsible' => 'hod_review_completed_by'],
                'QA Initial Review' => ['start' => 'hod_review_completed_on', 'end' => 'QA_Initial_Review_Complete_On', 'responsible' => 'QA_Initial_Review_Complete_by'],
                'CFT Review' => ['start' => 'QA_Initial_Review_Complete_On', 'end' => 'CFT_Review_Complete_On', 'responsible' => 'CFT_Review_Complete_by'],
                'QA Final Review' => ['start' => 'CFT_Review_Complete_On', 'end' => 'QA_Final_Review_Complete_On', 'responsible' => 'QA_Final_Review_Complete_by'],
                'QA Head/Manager Designee Approval' => ['start' => 'QA_Final_Review_Complete_On', 'end' => 'QA_head_approved_on', 'responsible' => 'QA_head_approved_by'],
                'Pending Initiator Update' => ['start' => 'QA_head_approved_on', 'end' => 'pending_initiator_approved_on', 'responsible' => 'pending_initiator_approved_by'],
                'QA Final Approval' => ['start' => 'pending_initiator_approved_on', 'end' => 'QA_final_approved_on', 'responsible' => 'QA_final_approved_by'],
                'Closed - Done' => ['start' => 'QA_final_approved_on', 'end' => 'QA_final_approved_on', 'responsible' => 'QA_final_approved_by'],
            ],

            'Lab Incident' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Pending Incident Verification' => ['start' => 'submitted_on', 'end' => 'verification_completed_on', 'responsible' => 'verification_completed_by'],
                'Pending Preliminary Investigation' => ['start' => 'verification_completed_on', 'end' => 'preliminary_completed_on', 'responsible' => 'preliminary_completed_by'],
                'Evaluation of Finding' => ['start' => 'preliminary_completed_on', 'end' => 'all_activities_completed_on', 'responsible' => 'all_activities_completed_by'],
                'Pending Solution & Sample Test' => ['start' => 'all_activities_completed_on', 'end' => 'no_assignable_cause_on', 'responsible' => 'no_assignable_cause_by'],
                'Pending Extended Investigation' => ['start' => 'no_assignable_cause_on', 'end' => 'review_completed_on', 'responsible' => 'review_completed_by'],
                'CAPA Initiation & Approval' => ['start' => 'review_completed_on', 'end' => 'extended_inv_complete_on', 'responsible' => 'extended_inv_complete_by'],
                'Final QA/Head Assessment' => ['start' => 'extended_inv_complete_on', 'end' => 'assesment_completed_on', 'responsible' => 'assesment_completed_by'],
                'Pending Approval' => ['start' => 'assesment_completed_on', 'end' => 'closure_completed_on', 'responsible' => 'closure_completed_by'],
                'Closed - Done' => ['start' => 'closure_completed_on', 'end' => 'closure_completed_on', 'responsible' => 'closure_completed_by'],
            ],

            'Incident' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'HOD Review' => ['start' => 'submit_on', 'end' => 'HOD_Review_Complete_On', 'responsible' => 'submit_by'],
                'QA Initial Review' => ['start' => 'HOD_Review_Complete_On', 'end' => 'QA_Initial_Review_Complete_On', 'responsible' => 'submit_by'],
                'CFT Review' => ['start' => 'QA_Initial_Review_Complete_On', 'end' => 'CFT_Review_Complete_On', 'responsible' => 'submit_by'],
                'QA Final Review' => ['start' => 'CFT_Review_Complete_On', 'end' => 'QA_Final_Review_Complete_On', 'responsible' => 'submit_by'],
                'QA Head/Manager Designee Approval' => ['start' => 'QA_Final_Review_Complete_On', 'end' => 'QA_head_approved_on', 'responsible' => 'submit_by'],
                'Pending Initiator Update' => ['start' => 'QA_head_approved_on', 'end' => 'pending_initiator_approved_on', 'responsible' => 'submit_by'],
                'QA Final Approval' => ['start' => 'pending_initiator_approved_on', 'end' => 'QA_final_approved_on', 'responsible' => 'submit_by'],
                'Closed - Done' => ['start' => 'QA_final_approved_on', 'end' => 'QA_final_approved_on', 'responsible' => 'submit_by'],
            ],

            'Action Item' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Acknowledge' => ['start' => 'submitted_on', 'end' => 'acknowledgement_on', 'responsible' => 'acknowledgement_by'],
                'Work Completion' => ['start' => 'acknowledgement_on', 'end' => 'work_completion_on', 'responsible' => 'work_completion_by'],
                'QA/CQA Verification' => ['start' => 'work_completion_on', 'end' => 'qa_varification_on', 'responsible' => 'qa_varification_by'],
                'Closed - Done' => ['start' => 'qa_varification_on', 'end' => 'qa_varification_on', 'responsible' => 'qa_varification_by'],
            ],

            'RCA' => [
                'Opened' => ['start' => 'created_at', 'end' => 'acknowledge_on', 'responsible' => 'acknowledge_by'],
                'HOD Review' => ['start' => 'acknowledge_on', 'end' => 'HOD_Review_Complete_On', 'responsible' => 'HOD_Review_Complete_by'],
                'Initial QA/CQA Review' => ['start' => 'HOD_Review_Complete_On', 'end' => 'QQQA_Review_Complete_On', 'responsible' => 'QQQA_Review_Complete_by'],
                'Investigation in Progress' => ['start' => 'QQQA_Review_Complete_On', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'HOD Final Review' => ['start' => 'submitted_on', 'end' => 'HOD_Final_Review_Complete_On', 'responsible' => 'HOD_Final_Review_Complete_by'],
                'Final QA/CQA Review' => ['start' => 'HOD_Final_Review_Complete_On', 'end' => 'Final_QA_Review_Complete_On', 'responsible' => 'Final_QA_Review_Complete_by'],
                'QAH/CQAH Final Review' => ['start' => 'Final_QA_Review_Complete_On', 'end' => 'evaluation_complete_on', 'responsible' => 'evaluation_complete_by'],
                'Closed - Done' => ['start' => 'evaluation_complete_on', 'end' => 'evaluation_complete_on', 'responsible' => 'evaluation_complete_by'],
            ],

            'Risk Assessment' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Risk Analysis & Work Group Assignment' => ['start' => 'submitted_on', 'end' => 'evaluated_on', 'responsible' => 'evaluated_by'],
                'Risk Processing & Action Plan' => ['start' => 'evaluated_on', 'end' => 'plan_complete_on', 'responsible' => 'plan_complete_by'],
                'Pending HOD Approval' => ['start' => 'plan_complete_on', 'end' => 'plan_approved_on', 'responsible' => 'plan_approved_by'],
                'Actions Items in Progress' => ['start' => 'plan_approved_on', 'end' => 'actions_completed_on', 'responsible' => 'actions_completed_by'],
                'Residual Risk Evaluation' => ['start' => 'actions_completed_on', 'end' => 'risk_analysis_completed_on', 'responsible' => 'risk_analysis_completed_by'],
                'Closed - Done' => ['start' => 'risk_analysis_completed_on', 'end' => 'risk_analysis_completed_on', 'responsible' => 'risk_analysis_completed_by'],
            ],

            'External Audit' => [
                'Opened' => ['start' => 'created_at', 'end' => 'audit_schedule_on', 'responsible' => 'audit_schedule_by'],
                'Audit Preparation' => ['start' => 'audit_schedule_on', 'end' => 'audit_preparation_completed_on', 'responsible' => 'audit_preparation_completed_by'],
                'Pending Audit' => ['start' => 'audit_preparation_completed_on', 'end' => 'audit_mgr_more_info_reqd_on', 'responsible' => 'audit_mgr_more_info_reqd_by'],
                'Pending Response' => ['start' => 'audit_mgr_more_info_reqd_on', 'end' => 'audit_observation_submitted_on', 'responsible' => 'audit_observation_submitted_by'],
                'CAPA Execution in Progress' => ['start' => 'audit_observation_submitted_on', 'end' => 'audit_response_completed_on', 'responsible' => 'audit_response_completed_by'],
                'Closed - Done' => ['start' => 'audit_response_completed_on', 'end' => 'audit_response_completed_on', 'responsible' => 'audit_response_completed_by'],
            ],
            ...
            'Audit Program' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Pending Approval' => ['start' => 'submitted_on', 'end' => 'approved_on', 'responsible' => 'approved_by'],
                'Pending Audit' => ['start' => 'approved_on', 'end' => 'Audit_Completed_On', 'responsible' => 'Audit_Completed_By'],
                'Closed - Done' => ['start' => 'Audit_Completed_On', 'end' => 'Audit_Completed_On', 'responsible' => 'Audit_Completed_By'],
            ],

            'New Document' => [
                'Initiate' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Pending Draft Creation' => ['start' => 'submitted_on', 'end' => 'approved_on', 'responsible' => 'approved_by'],
                'HOD Review' => ['start' => 'approved_on', 'end' => 'Audit_Completed_On', 'responsible' => 'Audit_Completed_By'],
                'QA Review' => ['start' => 'Audit_Completed_On', 'end' => 'Audit_Completed_On', 'responsible' => 'submit_by'],
                'Reviewer Review' => ['start' => 'created_at', 'end' => 'audit_schedule_on', 'responsible' => 'submit_by'],
                'Approver Pending' => ['start' => 'audit_schedule_on', 'end' => 'audit_preparation_completed_on', 'responsible' => 'submit_by'],
                'Pending-Traning' => ['start' => 'audit_preparation_completed_on', 'end' => 'audit_mgr_more_info_reqd_on', 'responsible' => 'submit_by'],
                'Traning Started' => ['start' => 'audit_mgr_more_info_reqd_on', 'end' => 'audit_observation_submitted_on', 'responsible' => 'submit_by'],
                'Traning-Complete' => ['start' => 'audit_observation_submitted_on', 'end' => 'audit_response_completed_on', 'responsible' => 'submit_by'],
                'Effective' => ['start' => 'audit_observation_submitted_on', 'end' => 'audit_response_completed_on', 'responsible' => 'submit_by'],
                'Closed - Done' => ['start' => 'audit_response_completed_on', 'end' => 'audit_response_completed_on', 'responsible' => 'submit_by'],
            ],

            'Effectiveness Check' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'Acknowledge' => ['start' => 'submit_on', 'end' => 'work_complition_on', 'responsible' => 'work_complition_by'],
                'Work Completion' => ['start' => 'work_complition_on', 'end' => 'effectiveness_check_complete_on', 'responsible' => 'effectiveness_check_complete_by'],
                'HOD Review' => ['start' => 'effectiveness_check_complete_on', 'end' => 'hod_review_complete_on', 'responsible' => 'hod_review_complete_by'],
                'QA/CQA Review' => ['start' => 'hod_review_complete_on', 'end' => 'effective_approval_complete_on', 'responsible' => 'effective_approval_complete_by'],
                'Closed - Effective' => ['start' => 'effective_approval_complete_on', 'end' => 'effective_approval_complete_on', 'responsible' => 'effective_approval_complete_by'],
            ],

            'Complaint Managment' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Supervisor Review' => ['start' => 'submitted_on', 'end' => 'complete_review_on', 'responsible' => 'complete_review_by'],
                'Investigation and Root Cause Analysis' => ['start' => 'complete_review_on', 'end' => 'investigation_completed_on', 'responsible' => 'investigation_completed_by'],
                'CAPA Plan' => ['start' => 'investigation_completed_on', 'end' => 'propose_plan_on', 'responsible' => 'propose_plan_by'],
                'Pending Approval' => ['start' => 'propose_plan_on', 'end' => 'approve_plan_on', 'responsible' => 'approve_plan_by'],
                'Pending Actions Completion' => ['start' => 'approve_plan_on', 'end' => 'all_capa_closed_on', 'responsible' => 'all_capa_closed_by'],
                'Pending Response Letter' => ['start' => 'all_capa_closed_on', 'end' => 'closed_done_on', 'responsible' => 'closed_done_by'],
                'Closed - Done' => ['start' => 'closed_done_on', 'end' => 'closed_done_on', 'responsible' => 'closed_done_by'],
            ],

            'Due Date Extension' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'In Review' => ['start' => 'submit_on', 'end' => 'submit_on_review', 'responsible' => 'submit_by_review'],
                'In Approved' => ['start' => 'submit_on_review', 'end' => 'submit_on_inapproved', 'responsible' => 'submit_by_inapproved'],
                'Closed - Reject' => ['start' => 'submit_on_inapproved', 'end' => 'submit_on_inapproved', 'responsible' => 'submit_by_inapproved'],
            ],

            'EHS & Environment Sustainability' => [
                'Opened' => ['start' => 'created_at', 'end' => 'Submit_On', 'responsible' => 'Submit_By'],
                'Pending Review' => ['start' => 'Submit_On', 'end' => 'Review_Complete_On', 'responsible' => 'Review_Complete_By'],
                'Pending Investigation' => ['start' => 'Review_Complete_On', 'end' => 'Complete_Investigation_On', 'responsible' => 'Complete_Investigation_By'],
                'Root Cause and Risk Analysis' => ['start' => 'Complete_Investigation_On', 'end' => 'Analysis_Complete_On', 'responsible' => 'Analysis_Complete_By'],
                'Pending Action Planning' => ['start' => 'Analysis_Complete_On', 'end' => 'Training_required_on', 'responsible' => 'Training_required_by'],
                'Pending Training' => ['start' => 'Training_required_on', 'end' => 'Training_complete_on', 'responsible' => 'Training_complete_by'],
                'Training Complete' => ['start' => 'Training_complete_on', 'end' => 'Propose_Plan_On', 'responsible' => 'Propose_Plan_By'],
                'Pending Approval' => ['start' => 'Propose_Plan_On', 'end' => 'Approve_Plan_On', 'responsible' => 'Approve_Plan_By'],
                'CAPA Execution in Progres' => ['start' => 'Approve_Plan_On', 'end' => 'All_CAPA_Closed_On', 'responsible' => 'All_CAPA_Closed_By'],
                'Closed - Done' => ['start' => 'All_CAPA_Closed_On', 'end' => 'All_CAPA_Closed_On', 'responsible' => 'All_CAPA_Closed_By'],

            ],

            'Equipment Lifecycle Management' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'Supervisor Review' => ['start' => 'submit_on', 'end' => 'Supervisor_Approval_on', 'responsible' => 'Supervisor_Approval_by'],
                'Pending Qualification' => ['start' => 'Supervisor_Approval_on', 'end' => 'Complete_Qualification_on', 'responsible' => 'Complete_Qualification_by'],
                'Pending Training' => ['start' => 'Complete_Qualification_on', 'end' => 'Training_required_on', 'responsible' => 'Training_required_by'],
                'Training Evalution' => ['start' => 'Training_required_on', 'end' => 'Complete_Training_on', 'responsible' => 'Complete_Training_by'],
                'Pending QA Approval' => ['start' => 'Complete_Training_on', 'end' => 'Take_Out_of_Service_on', 'responsible' => 'Take_Out_of_Service_by'],
                'Active Equipment' => ['start' => 'Take_Out_of_Service_on', 'end' => 'Forward_to_Storage_on', 'responsible' => 'Forward_to_Storage_by'],
                'Out of Service' => ['start' => 'Forward_to_Storage_on', 'end' => 'forword_storage_on', 'responsible' => 'forword_storage_by'],
                'In Storage' => ['start' => 'forword_storage_on', 'end' => 'retire_on', 'responsible' => 'retire_by'],
                'Closed - Retired' => ['start' => 'retire_on', 'end' => 'retire_on', 'responsible' => 'retire_by'],
            ],

            'ERRATA' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Pending Review' => ['start' => 'submitted_on', 'end' => 'review_completed_on', 'responsible' => 'review_completed_by'],
                'Pending Correction' => ['start' => 'review_completed_on', 'end' => 'correction_completed_on', 'responsible' => 'correction_completed_by'],
                'Pending HOD Review' => ['start' => 'correction_completed_on', 'end' => 'hod_review_complete_on', 'responsible' => 'hod_review_complete_by'],
                'Pending QA Head Approval' => ['start' => 'hod_review_complete_on', 'end' => 'qa_head_approval_completed_on', 'responsible' => 'qa_head_approval_completed_by'],
                'Closed-Done' => ['start' => 'qa_head_approval_completed_on', 'end' => 'qa_head_approval_completed_on', 'responsible' => 'qa_head_approval_completed_by'],
            ],
            ...
            'Global CAPA' => [
                'Opened' => ['start' => 'created_at', 'end' => 'plan_proposed_on', 'responsible' => 'plan_proposed_by'],
                'HOD Review' => ['start' => 'plan_proposed_on', 'end' => 'HOD_Review_Complete_On', 'responsible' => 'HOD_Review_Complete_By'],
                'QA/CQA Review' => ['start' => 'HOD_Review_Complete_On', 'end' => 'qa_review_completed_on', 'responsible' => 'qa_review_completed_by'],
                'QA/CQA Approval' => ['start' => 'qa_review_completed_on', 'end' => 'approved_on', 'responsible' => 'approved_by'],
                'CAPA In progress' => ['start' => 'approved_on', 'end' => 'completed_on', 'responsible' => 'completed_by'],
                'HOD Final Review' => ['start' => 'completed_on', 'end' => 'hod_final_review_completed_on', 'responsible' => 'hod_final_review_completed_by'],
                'QA/CQA Closure Review' => ['start' => 'hod_final_review_completed_on', 'end' => 'qa_closure_review_completed_on', 'responsible' => 'qa_closure_review_completed_by'],
            ...
            'Preventive Maintenance' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submit_on', 'responsible' => 'submit_by'],
                'Supervisor Review' => ['start' => 'submit_on', 'end' => 'Supervisor_Approval_on', 'responsible' => 'Supervisor_Approval_by'],
                'Work in Progress' => ['start' => 'Supervisor_Approval_on', 'end' => 'Complete_on', 'responsible' => 'Complete_by'],
                'Pending QA Approval' => ['start' => 'Complete_on', 'end' => 'qa_approval_on', 'responsible' => 'qa_approval_by'],
                'Closed-Done' => ['start' => 'qa_approval_on', 'end' => 'qa_approval_on', 'responsible' => 'qa_approval_by'],
            ],

            'Meeting' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'In Progress' => ['start' => 'submitted_on', 'end' => 'complete_on', 'responsible' => 'complete_by'],
                'Closed-Done' => ['start' => 'complete_on', 'end' => 'complete_on', 'responsible' => 'complete_by'],
            ],

            'Sanction' => [
                'Opened' => ['start' => 'created_at', 'end' => 'Cancel_On', 'responsible' => 'Cancel_By'],
                'Closed' => ['start' => 'Cancel_On', 'end' => 'Cancel_On', 'responsible' => 'Cancel_By'],
            ],

            'Supllier' => [
                'Opened' => ['start' => 'created_at', 'end' => 'submitted_on', 'responsible' => 'submitted_by'],
                'Pending Initiating Department Update' => ['start' => 'submitted_on', 'end' => 'request_justified_on', 'responsible' => 'request_justified_by'],
                'Pending Update FROM CQA' => ['start' => 'request_justified_on', 'end' => 'prepurchase_sample_on', 'responsible' => 'prepurchase_sample_by'],
                'Pending Purchase Sample Request' => ['start' => 'prepurchase_sample_on', 'end' => 'pendigPurchaseSampleRequested_on', 'responsible' => 'pendigPurchaseSampleRequested_by'],
                'Pending CQA Review After Purchase Sample Request' => ['start' => 'pendigPurchaseSampleRequested_on', 'end' => 'purchaseSampleanalysis_on', 'responsible' => 'purchaseSampleanalysis_by'],
                'Pending F&D Review' => ['start' => 'purchaseSampleanalysis_on', 'end' => 'FdReviewCompleted_on', 'responsible' => 'FdReviewCompleted_by'],
                'Pending Acknowledgement By Purchase Department' => ['start' => 'FdReviewCompleted_on', 'end' => 'acknowledgByPD_on', 'responsible' => 'acknowledgByPD_by'],
                'Pending CQA Final Review' => ['start' => 'acknowledgByPD_on', 'end' => 'requirementFullfilled_on', 'responsible' => 'requirementFullfilled_by'],
                'Pending Manufacturer Risk Assessment' => ['start' => 'requirementFullfilled_on', 'end' => 'riskRatingObservedAsHigh_on', 'responsible' => 'riskRatingObservedAsHigh_by'],
                'Pending Manufacturer Audit' => ['start' => 'riskRatingObservedAsHigh_on', 'end' => 'manufacturerAuditPassed_on', 'responsible' => 'manufacturerAuditPassed_by'],
                'Approved Manufacturer/Supplier' => ['start' => 'manufacturerAuditPassed_on', 'end' => 'periodicRevolutionInitiated_on', 'responsible' => 'periodicRevolutionInitiated_by'],
                 ],
        ];
        ...
        if ($process) {
            switch ($process) {
                ...
                case 'Equipment Lifecycle Management':
            ...
            if ($query) {
                ...
                foreach ($records as $record) {
                    ...
                    foreach ($fieldMappings[$process] as $stage => $fields) {
                        ...
                        $responsible = $record->{$fields['responsible']} ?? null; 
                    ...
                    $record->active_stage_number = $record->stage ?? null; 
                    $record->active_stage = $record->status ?? null; 
