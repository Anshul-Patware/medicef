...
if ($incident->Initial_attachment) {
    ...
    // $files = is_array(json_decode($NonConformance->Audit_file)) ? $NonConformance->Audit_file : [];
        ...
        // $userNotification->record_id = $incident->id;
        // $userNotification->record_type = "Incident";
    ...
    public function update(Request $request, $id)
    {
        ...
        $incident->Description_of_Discrepancy = $request->Description_of_Discrepancy;
        $incident->capa_root_cause = $request->capa_root_cause;
        $incident->Immediate_Action_Take = $request->Immediate_Action_Take;
        $incident->Corrective_Action_Details = $request->Corrective_Action_Details;
        $incident->Preventive_Action_Details = $request->Preventive_Action_Details;
        $incident->capa_completed_date = $request->capa_completed_date;
        $incident->Interim_Control = $request->Interim_Control;
        $incident->Corrective_Action_Taken = $request->Corrective_Action_Taken;
        $incident->Preventive_action_Taken = $request->Preventive_action_Taken;
        $incident->CAPA_Closure_Comments = $request->CAPA_Closure_Comments;
        ...
        if (!empty($request->risk_factor)) {
            $incident->risk_factor = serialize($request->risk_factor);
        }
        if (!empty($request->risk_element)) {
            $incident->risk_element = serialize($request->risk_element);
        }
        if (!empty($request->problem_cause)) {
            $incident->problem_cause = serialize($request->problem_cause);
        }
        if (!empty($request->existing_risk_control)) {
            $incident->existing_risk_control = serialize($request->existing_risk_control);
        }
        if (!empty($request->initial_severity)) {
            $incident->initial_severity = serialize($request->initial_severity);
        }
        if (!empty($request->initial_detectability)) {
            $incident->initial_detectability = serialize($request->initial_detectability);
        }
        if (!empty($request->initial_probability)) {
            $incident->initial_probability = serialize($request->initial_probability);
        }
        if (!empty($request->initial_rpn)) {
            $incident->initial_rpn = serialize($request->initial_rpn);
        }
        if (!empty($request->risk_acceptance)) {
            $incident->risk_acceptance = serialize($request->risk_acceptance);
        }
        if (!empty($request->risk_control_measure)) {
            $incident->risk_control_measure = serialize($request->risk_control_measure);
        }
        if (!empty($request->residual_severity)) {
            $incident->residual_severity = serialize($request->residual_severity);
        }
        if (!empty($request->residual_probability)) {
            $incident->residual_probability = serialize($request->residual_probability);
        }
        if (!empty($request->residual_detectability)) {
            $incident->residual_detectability = serialize($request->residual_detectability);
        }
        if (!empty($request->residual_rpn)) {
            $incident->residual_rpn = serialize($request->residual_rpn);
        }
        if (!empty($request->risk_acceptance2)) {
            $incident->risk_acceptance2 = serialize($request->risk_acceptance2);
        }
        if (!empty($request->mitigation_proposal)) {
            $incident->mitigation_proposal = serialize($request->mitigation_proposal);
        }
        ...
        $incident->rpn = $request->rpn ? $request->rpn: $incident->rpn;
    ...
    {
        $data = Incident::find($id);
        return view('frontend.incident.incident_showpdf', compact('id', 'data'));
    }
     ...
     public static function activityLog($id)
     {
        ...
        $data1 =  IncidentCft::where('incident_id', $id)->first();
         if (!empty ($data)) {
             ...
             $pdf = PDF::loadview('frontend.incident.activity_report', compact('data','data1'))
                 ->setOptions([
                 'defaultFont' => 'sans-serif',
                 'isHtml5ParserEnabled' => true,
                 'isRemoteEnabled' => true,
