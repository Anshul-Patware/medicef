...
class OOTController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data->nature_of_change = is_array($request->nature_of_change)  ? implode(',', $request->nature_of_change)  : $request->nature_of_change;
    ...
    //    dd($grid_product_mat);
       ...
       $checkList =OotChecklist::where(['ootcs_id' => $id])->first();
        ...
        if(!empty($data)){
            $pdf = App::make('dompdf.wrapper');
            $time = Carbon::now();
            $pdf = PDF::loadview('frontend.OOT.oot_family_report', compact('data','grid_product_mat','gridStability','GridOotRes','InfoProductMat','checkList'))
                ->setOptions([
                'defaultFont' => 'sans-serif',
                'isHtml5ParserEnabled' => true,
                'isRemoteEnabled' => true,
