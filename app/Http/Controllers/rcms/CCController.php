...
class CCController extends Controller
{
    public function create()
    {
        ...
        $cft = User::get();
        $pre = CC::get();
        $allPreviousRecords = ChangeControlFields::all(['similarChangeControl']);

        $currentYear = date('Y');
        $getCCCount = CC::whereYear('created_at', $currentYear)->count();
        $changeControlRecord = $getCCCount + 1;

        // $getCCCount = CC::count();
        // $changeControlRecord = $getCCCount + 1;
        return view('frontend.change-control.new-change-control', compact("riskData", "preRiskAssessment", "due_date", "hod", "cft", "pre", "changeControlRecord", "allPreviousRecords"));
    }
    ...
    public function store(Request $request)
    {
        ...
        $openState->record_number = "CRF/" . $request->changeControlNature . "/" . substr(date('y'), -2) . "/" . str_pad($request->changeControlCount, 3, '0', STR_PAD_LEFT);
       ...
       $openState->Detection = $request->Detection;
        ...
        $fields->similarChangeControlRequired = $request->similarChangeControlRequired;
        $fields->similarChangeControl = implode(',',$request->similarChangeControl);
        $fields->changeControlCount = $request->changeControlCount;
        if(!empty($request->changeControlNatureInput)){
            $fields->changeControlNature = $request->changeControlNatureInput;
        } else {
            $fields->changeControlNature = $request->changeControlNature;
        }
        $fields->save();
    ...
    public function update(Request $request, $id)
    {
        ...
        $CCfields->similarChangeControlRequired = $request->similarChangeControlRequired;
        $CCfields->similarChangeControl = implode(',',$request->similarChangeControl);
        // $CCfields->changeControlNature = $request->changeControlNature;
        if(!empty($request->changeControlNatureInput)){
            $CCfields->changeControlNature = $request->changeControlNatureInput;
        } else {
            $CCfields->changeControlNature = $request->changeControlNature;
        }
...
if ($lastCft->Other3_on != $request->Other3_on && $request->Other3_on != null) {
    ...
    public function sentoPostImplementation(Request $request, $id)
    {
        ...
        if ($changeControl->stage == 12) {
            if (is_null($lastDocument->qa_closure_comments))
            {
                Session::flash('swal', [
                    'type' => 'warning',
                    'title' => 'Mandatory Fields!',
