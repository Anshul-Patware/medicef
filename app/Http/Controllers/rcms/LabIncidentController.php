...
class LabIncidentController extends Controller
{
    ...
    public function create(request $request)
    {
        ...
        $data->priority_data = $request->priority_data;
        ...
        // $userNotification->record_type = "Lab Incident";
        ...
        $history->current = Helpers::getDivisionName(session()->get('division')) . "/LI/" . date('Y') . "/" . str_pad($data->record, 4, '0', STR_PAD_LEFT);
        ...
        if(!empty($data->priority_data)) {
            $history = new LabIncidentAuditTrial();
            $history->LabIncident_id = $data->id;
            $history->activity_type = 'Priority';
            $history->previous = "Null";
            $history->current = $data->priority_data;
        ...
        $initiatorGroup = Helpers::getNewInitiatorGroupFullName($data->Initiator_Group);
        ...
        if (!empty($data->product_name)) {
            ...
            $history->previous = "Null";
            $history->current = $data->product_name;
            $history->comment = "Not Applicable";
            $history->user_id = Auth::user()->id;
            $history->user_name = Auth::user()->name;
            $history->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
        ...
        if (!empty($data->type_incidence_ia)) {
            ...
            $history->previous = "Null";
            $history->current = $request->type_incidence_ia;
            $history->comment = "Not Applicable";
            $history->user_id = Auth::user()->id;
            $history->user_name = Auth::user()->name;
            $history->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
    ...
    public function updateLabIncident(request $request, $id)
    {
       ...
       if($request->has('extension_date_idsc')&& $request->extension_date_idsc!==null){
        ...
        if (!is_null($lastDocument->analyst_sign_date_gi) != $data['analyst_sign_date_gi']||$lastDocument->analyst_sign_date_gi != $data['analyst_sign_date_gi']){
            $history = new LabIncidentAuditTrial();
            $history->LabIncident_id = $data['id'];
        ...
        if ($lastDocument->priority_data != $data->priority_data ) {
        ...
        if (!is_null($lastDocument->assign_to) != $data['assign_to']) {
            $history = new LabIncidentAuditTrial();
            $history->LabIncident_id = $id;
            $history->activity_type = 'Assigned to';
        ...
        if ($lastDocument->Incident_specification_ssfi != $data->Incident_specification_ssfi) {
            ...
            $history->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
            $history->change_from = $lastDocument->status;
            if (is_null($lastDocument->Incident_specification_ssfi) || $lastDocument->Incident_specification_ssfi === '') {
                $history->action_name = "New";
            } else {
                $history->action_name = "Update";
            }
        ...
        if ($lastDocument->type_incidence_ia != $data->type_incidence_ia) {
            ...
            $history->comment = $request->type_incidence_ia_comment;
            $history->user_id = Auth::user()->id;
            $history->user_name = Auth::user()->name;
            $history->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
            $history->change_to = "Not Applicable";
...
if ($lastDocument->ccf_attachments != $data->ccf_attachments) {
        ...
        return view('frontend.labIncident.view', compact('data','report','systemSutData','labnew'));
           ...
           public function lab_incident_capa_child(Request $request, $id)
           {
               ...
               if ($request->revision == "Action-Item") {
                    ...
                    $parentRecord = LabIncident::where('id', $id)->value('record');
                    $expectedParenRecord = Helpers::getDivisionName(session()->get('division')) . "/LI/" . date('Y') . "/" . $cc->record . "";
                   return view('frontend.action-item.action-item', compact('expectedParenRecord','record_number', 'due_date','old_record', 'parent_id','record', 'parent_type','parent_intiation_date','parent_record','parent_initiator_id'));

               }

               if ($request->revision == "capa-child") {
                   $cc->originator = User::where('id', $cc->initiator_id)->value('name');
                  return view('frontend.forms.capa', compact('record_number', 'due_date','old_records', 'parent_id', 'parent_type', 'old_record', 'cft'));
               }
               if ($request->revision == "Extension") {
                $relatedRecords = Helpers::getAllRelatedRecords();
                $cc->originator = User::where('id', $cc->initiator_id)->value('name');
                return view('frontend.extension.extension_new', compact('record_number','relatedRecords','due_date', 'parent_id', 'parent_type','parent_intiation_date','parent_record','parent_initiator_id'));
           ...
           public function lab_incident_extension_child(Request $request, $id)
           {
               ...
               if ($request->revision == "Extension") {
                   $cc->originator = User::where('id', $cc->initiator_id)->value('name');
                   return view('frontend.extension.extension_new', compact('record_number', 'due_date', 'parent_id', 'parent_type','parent_intiation_date','relatedRecords','parent_record','parent_initiator_id'));
    ...
    public function lab_incident_root_child(Request $request, $id)
    {
        ...
        if ($request->revision == "Action-Item") {
            ...
            return view('frontend.action-it', compact('record_number','old_record','due_date', 'parent_id', 'parent_type','parent_intiation_date','parent_record','parent_initiator_id'));
        ...
        if ($request->revision == "effectiveness-check") {
         $cc->originator = User::where('id', $cc->initiator_id)->value('name');
           return view('frontend.forms.effectiveness-check', compact( 'parent_id', 'parent_type','record_number','currentDate','old_record','formattedDate','due_date', 'parent_record'));
        // return view('frontend.forms.root-cause-analysis', compact('record_number', 'due_date', 'parent_id', 'parent_type'));
    };
    if ($request->revision == "Extension") {
        $cc->originator = User::where('id', $cc->initiator_id)->value('name');
        return view('frontend.extension.extension_new', compact('record_number', 'due_date', 'parent_id','old_record', 'parent_type','parent_intiation_date','parent_record','parent_initiator_id'));
    ...
    public function LabIncidentStateTwo(Request $request,$id)
    {
        if ($request->username == Auth::user()->Username && Hash::check($request->password, Auth::user()->password)) {
            ...
            $history->activity_type = 'No Assignable Cause Identification By, No Assignable Cause Identification On';
                ...
                if (is_null($lastDocument->no_assignable_cause_by) || $lastDocument->no_assignable_cause_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
            ...
            $history->save();
             ...
             /********** Notification User **********/
             $list = Helpers::getHodUserList($labstate->division_id);
             ...
             if(!empty($users)){
                 try {
                     ...
                     $history->change_to = "Not Applicable";
                     $history->change_from = "Pending Extended Investigation";
                     $history->stage = "";
                     $history->action_name = "";
             ...
             foreach ($users as $userValue) {
                 DB::table('notifications')->insert([
                     'activity_id' => $labstate->id,
                     'activity_type' => "Notification",
                     'from_id' => Auth::user()->id,
                     'user_name' => $userValue->name,
                     'to_id' => $userValue->id,
                     'process_name' => "Lab Incident",
                     'division_id' => $labstate->division_id,
                     'short_description' => $labstate->short_description,
                     'initiator_id' => $labstate->initiator_id,
                     'due_date' => $labstate->due_date,
                     'record' => $labstate->record,
                     'site' => "LI",
                     'comment' => $request->comments,
                     'status' => $labstate->status,
                     'stage' => $labstate->stage,
             ...
             foreach ($list as $u) {
                 $email = Helpers::getUserEmail($u->user_id);
                     ...
                     try {
                         Mail::send(
                             'mail.view-mail',
                             ['data' => $labstate, 'site' => "LI", 'history' => "No Assignable Cause Identification", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                             function ($message) use ($email, $labstate) {
                                 $message->to($email)
                                 ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($labstate->record, 4, '0', STR_PAD_LEFT) . " - Activity: No Assignable Cause Identification Performed");
                             }
                         );
                     } catch(\Exception $e) {
    ...
    public function LabIncidentStateChange(Request $request, $id)
    {
        if ($request->username == Auth::user()->Username && Hash::check($request->password, Auth::user()->password)) {
            ...
            if ($changeControl->stage == 1) {
                ...
                if(!empty($users)){
                    try {
                        ...
                        $history->origin_state = "Not Applicable";
                        $history->change_to = "Not Applicable";
                        $history->change_from = "Pending Incident Verification";
                        $history->stage = "";
                        $history->action_name = "";
                        $history->mailUserId = $userIds;
                        $history->role_name = "Initiator";
                        ...
                        try {
                            Mail::send(
                                ...
                                function ($message) use ($email, $changeControl) {
                                    $message->to($email)
                                    ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Submit Performed");
                                }
                            );
                        } catch(\Exception $e) {
                            info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 2) {
                ...
                if (is_null($lastDocument->verification_complete_completed_by) || $lastDocument->verification_completed_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
                ...
                $history->save();
                 ...
                 if(!empty($users)){
                     try {
                         ...
                         $history->origin_state = "Not Applicable";
                         $history->change_to = "Not Applicable";
                         $history->change_from = "Pending Preliminary Investigation";
                         $history->stage = "";
                         $history->action_name = "";
                         $history->mailUserId = $userIds;
                         $history->role_name = "Initiator";
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "Verification Complete", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Verification Complete Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 3) {
                ...
                $history->activity_type = 'Preliminary Investigation by, Preliminary Investigation on';
                ...
                if (is_null($lastDocument->preliminary_completed_by) || $lastDocument->preliminary_completed_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
                ...
                $history->save();
                 ...
                 if(!empty($users)){
                     try {
                         ...
                         $history->origin_state = "Not Applicable";
                         $history->change_to = "Not Applicable";
                         $history->change_from = "Evaluation of Finding";
                         $history->stage = "";
                         $history->action_name = "";
                         $history->mailUserId = $userIds;
                         $history->role_name = "Initiator";
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "Preliminary Investigation", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Preliminary Investigation Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
                ...
                if ($changeControl->stage == 4) {
                    ...
                    $history->activity_type = 'Assignable Cause Identification by, Assignable Cause Identification on';
                    ...
                    if (is_null($lastDocument->all_activities_completed_by) || $lastDocument->all_activities_completed_on === '') {
                        $history->action_name = 'New';
                    } else {
                        $history->action_name = 'Update';
                    }
                 ...
                 if(!empty($users)){
                     try {
                         ...
                         $history->origin_state = "Not Applicable";
                         $history->change_to = "Not Applicable";
                         $history->change_from = "Pending Solution & Sample Test";
                         $history->stage = "";
                         $history->action_name = "";
                         $history->mailUserId = $userIds;
                         $history->role_name = "Initiator";
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "Assignable Cause Identification", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Assignable Cause Identification Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 5) {
                ...
                $history->activity_type = 'Solution Validation by, Solution Validation on';
                ...
                $history->save();
                      ...
                      if(!empty($users)){
                          try {
                              ...
                              $history->origin_state = "Not Applicable";
                              $history->change_to = "Not Applicable";
                              $history->change_from = "CAPA Initiation & Approval";
                              $history->stage = "";
                              $history->action_name = "";
                              $history->mailUserId = $userIds;
                              $history->role_name = "Initiator";
                      ...
                      foreach ($list as $u) {
                          $email = Helpers::getUserEmail($u->user_id);
                              ...
                              try {
                                  Mail::send(
                                      'mail.view-mail',
                                      ['data' => $changeControl, 'site' => "LI", 'history' => "Solution Validation", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                      function ($message) use ($email, $changeControl) {
                                          $message->to($email)
                                          ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Solution Validation Performed");
                                      }
                                  );
                              } catch(\Exception $e) {
                                  info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 6) {
                ...
                $history->activity_type = 'Extended Inv. Complete by, Extended Inv. Complete on';
                ...
                if (is_null($lastDocument->extended_inv_complete_by) || $lastDocument->extended_inv_complete_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
                ...
                $history->save();
                 ...
                 if(!empty($users)){
                     try {
                         ...
                         $history->origin_state = "Not Applicable";
                         $history->change_to = "Not Applicable";
                         $history->change_from = "Final QA/Head Assessment";
                         $history->stage = "";
                         $history->action_name = "";
                         $history->mailUserId = $userIds;
                         $history->role_name = "Initiator";
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "Extended Inv. Complete", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Extended Inv. Complete Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 7) {
                ...
                $history->activity_type = 'All Action Approved by, All Action Approved on';
                ...
                $history->current = $changeControl->extended_inv_complete_by . ' , ' . $changeControl->extended_inv_complete_on;
                if (is_null($lastDocument->all_actiion_approved_by) || $lastDocument->extended_inv_complete_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "All Action Approved", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: All Action Approved Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 8) {
                ...
                $history->activity_type = 'Assessment Completed by, Assessment Completed on';
                ...
                if (is_null($lastDocument->assesment_completed_by) || $lastDocument->assesment_completed_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "Assessment Completed", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Assessment Completed Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
            ...
            if ($changeControl->stage == 9) {
                ...
                $history->activity_type = 'Closure Completed by, Closure Completed on';
                ...
                if (is_null($lastDocument->closure_completed_by) || $lastDocument->closure_completed_on === '') {
                    $history->action_name = 'New';
                } else {
                    $history->action_name = 'Update';
                }
                ...
                $history->action='Closure Completed';
                $history->save();
                 /********** Notification User **********/
                 $list = Helpers::getHodUserList($changeControl->division_id);
                 $userIds = collect($list)->pluck('user_id')->toArray();
                 ...
                 foreach ($list as $u) {
                     $email = Helpers::getUserEmail($u->user_id);
                         ...
                         try {
                             Mail::send(
                                 'mail.view-mail',
                                 ['data' => $changeControl, 'site' => "LI", 'history' => "Closure Completed", 'process' => 'Lab Incident', 'comment' => $request->comment, 'user' => Auth::user()->name],
                                 function ($message) use ($email, $changeControl) {
                                     $message->to($email)
                                     ->subject("Medicef Notification: Lab Incident, Record #" . str_pad($changeControl->record, 4, '0', STR_PAD_LEFT) . " - Activity: Closure Completed Performed");
                                 }
                             );
                         } catch(\Exception $e) {
                             info('Error sending mail', [$e]);
    ...
    public static function LabIncidentActivityLogPdf($id){
        ...
        if (!empty ($data)) {
            ...
            $pdf = PDF::loadview('frontend.labIncident.lab_incident_activity_pdf', compact('data'))
                ->setOptions([
    ...
    public static function singleReport($id)
    {
        ...
        if (!empty($data)) {
            ...
            $pdf = App::make('dompdf.wrapper');
            $time = Carbon::now();
            $pdf = PDF::loadview('frontend.labIncident.singleReport', compact('data','labtab','labgrid','labtab_grid','labnew'))
                ->setOptions([
                    'defaultFont' => 'sans-serif',
                    'isHtml5ParserEnabled' => true,
                    'isRemoteEnabled' => true,
    ...
    {
        $data = LabIncident::find($id);
        return view('frontend.labIncident.labincident_showpdf', compact('id', 'data'));
    }
    ...
    public static function familyReport($id)
    {
        ...
        if (!empty($data)) {
            ...
            $RiskAssessment =  RiskManagement::where('parent_id', $id)->get();
            ...
            $pdf = App::make('dompdf.wrapper');
            $time = Carbon::now();
            $pdf = PDF::loadview('frontend.labIncident.labincident_family_report', compact('data','labnew','Extension','capa_teamNamesString','capa','ActionItem','labtab','labgrid','labtab_grid', 'EffectivenessCheck', 'RiskAssessment','riskgrdfishbone', 'riskgrdwhy_chart', 'riskgrdwhat_who_where'))
                ->setOptions([
                    'defaultFont' => 'sans-serif',
                    'isHtml5ParserEnabled' => true,
                    'isRemoteEnabled' => true,
