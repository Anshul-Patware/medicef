    ...
    public function index()
    {
        ...
        LabIncident::all()->map(function ($query) use (&$due_dates, $today) {
            $due_dates[] = [
                ...
                'start' => Carbon::parse($query->due_date)->toDateString(),
                'backgroundColor' => Carbon::parse($query->due_date)->subDays(2)->lt($today) ? 'red' : 'green',
                'url' => url('rcms/LabIncidentShow', ['id' => $query->id])
            ];
        });
        ...
        CallibrationDetails::all()->map(function ($query) use (&$due_dates, $today) {
            $due_dates[] = [
                'Form_Type' => 'Calibration Details',
                'title' => Helpers::getDivisionCode($query->division_id) . '/Calibration Details/' . date('Y') . '/' . str_pad($query->record, 4, '0', STR_PAD_LEFT),
                'start' => Carbon::parse($query->due_date)->toDateString(),
                'backgroundColor' => Carbon::parse($query->due_date)->subDays(2)->lt($today) ? 'red' : 'green',
                'url' => url('showCalibrationDetails', ['id' => $query->id]) 
            ];
        });
    ...
    public function analyticsData(Request $request)
    {
        if ($request->value == "due") {
            ...
            $data = [
                ...
                'Extension' => extension_new::whereDate('due_date', '<', $current_date)->count(),
