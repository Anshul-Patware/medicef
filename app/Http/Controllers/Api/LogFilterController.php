    ...
    public function OOT_Filter(Request $request)
{
    ...
    public function errata_filter(Request $request)
    {
        ...
        try {
            ...
            {
                $query->whereIn('department_code', $request->department_e);
            }
            ...
            if ($request->initiator_id_e) {
                $query->whereIn('initiator_id', $request->initiator_id_e);
            }
...
public function extension_filter(Request $request)
{
    ...
    try {
        ...
        if (!empty($request->department_extension)) {
            $query->whereIn('priority_data', $request->department_extension);  // Use whereIn for multiple selections
        }
        
        
        if ($request->division_extension) {
            $query->where('division_id', $request->division_extension);
        }
...
public function printPDFExtension (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.extension_pdf', compact('FilterDDD','totalRows','totalPages','paginatedData'))->setPaper('a4', 'landscape');
...
public function effectivness(Request $request)
{
    ...
    try {
        ...
        if ($request->initiator_id_effectiveness) {
         ...
         if ($request->date_fromeffectivness) {
            $dateFrom = Carbon::parse($request->date_fromeffectivness)->startOfDay();
            $query->whereDate('created_at', '>=', $dateFrom);
        }
        
        if ($request->date_toeffectiveness) {
            $dateTo = Carbon::parse($request->date_toeffectiveness)->endOfDay();
            $query->whereDate('created_at', '<=', $dateTo);
        }
    ...
    public function printPDFEffectivenesss (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.filterData.effectiveness_pdf', compact('FilterDDD','paginatedData','totalPages','totalRows'))->setPaper('a4', 'landscape');
...
public function printPDFLab (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.lablogprintFile', compact('FilterDDD'))->setPaper('a4', 'landscape');
...
public function printPDFDeviation (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.deviation_log_pdf', compact('FilterDDD','totalPages','paginatedData','totalRows'))->setPaper('a4', 'landscape');
...
public function printPDFriskmt (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.riskmanagement_log_pdf', compact('FilterDDD'))->setPaper('a4', 'landscape');
...
public function printPDFoot (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.oot_log_pdf', compact('FilterDDD'))->setPaper('a4', 'landscape');
...
public function printPDFerrata (Request $request)
{
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.errata_log_pdf', compact('FilterDDD','rowsPerPage','totalPages','paginatedData'))->setPaper('a4', 'landscape');
...
public function equipmentlifecycle_filter(Request $request)
    {
        ...
        try {
            ...
            if ($request->department_elm) {
                $query->whereIn('criticality_level', $request->department_elm);
            }
            
            if ($request->division_id_elm) {
                $query->where('division_id', $request->division_id_elm);
            }

            if ($request->initiator_id_elm) {
                $query->whereIn('initiator_id', $request->initiator_id_elm);
            }
    
            if ($request->date_fromelm) {
                $dateFrom = Carbon::parse($request->date_fromelm)->startOfDay();
                $query->whereDate('intiation_date', '>=', $dateFrom);
            }

            if ($request->date_toelm) {
                $dateTo = Carbon::parse($request->date_toelm)->endOfDay();
                $query->whereDate('intiation_date', '<=', $dateTo);
            }
            if ($request->sort_column && $request->sort_order) {
                $query->orderBy($request->sort_column, $request->sort_order);
    ...
    public function calibrationmanagement_filter(Request $request)
    {
        ...
        try {
            ...
            if (!empty($request->department_calibrationmanagement)) {
                $query->whereIn('callibration_frequency', $request->department_calibrationmanagement); 
            }
...
public function ehs_filter(Request $request)
{
    $res = [
        'status' => 'ok',
        'message' => 'success',
    ...
    try {
        ...
        // Handle department filter when multiple departments are selected
        if (!empty($request->department_ehsevent)) {
            $query->whereIn('Department', $request->department_ehsevent);  // Use whereIn for multiple selections297
        }

        if ($request->division_id_ehsevent) {
            $query->where('division_id', $request->division_id_ehsevent);
        }

        if ($request->initiator_id_ehseventehsevent) {
            $query->where('initiator_id', $request->initiator_id_ehseventehsevent);
        }

        if ($request->date_fromehsevent) {
            $dateFrom = Carbon::parse($request->date_fromehsevent)->startOfDay();
            $query->whereDate('created_at', '>=', $dateFrom);
        }

        if ($request->date_toehsevent) {
            $dateTo = Carbon::parse($request->date_toehsevent)->endOfDay();
            $query->whereDate('created_at', '<=', $dateTo);
        }
        ...
        $htmlData = view('frontend\forms.Logs.filterData.ehs_event_data', compact('ehs'))->render();
...
public function printReportehseventLog(Request $request)
{
    $filterCC = $request->all();
    ...
    $pdf = Pdf::loadView('frontend.forms.Logs.filterData.ehs_log_pdf', compact('rowsPerPage','paginatedData','totalPages','filteredDataCC'))->setPaper('a4', 'landscape');
