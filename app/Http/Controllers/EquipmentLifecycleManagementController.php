...
public function EquipmentStateChange(Request $request, $id)
    {
        if ($request->username == Auth::user()->Username && Hash::check($request->password, Auth::user()->password)) {
            ...
            if ($equipment->stage == 1) {
                ...
                $history->save();
                 ...
                 foreach ($users as $userValue) {
                     DB::table('notifications')->insert([
                         ...
                         'to_id' => $userValue->id,
                         'process_name' => "Equipment Lifecycle Management",
                         ...
                         'site' => "Equipment Lifecycle Management",
                ...
                //              Mail::send(
                //                  'mail.view-mail',
                //                  ['data' => $equipment, 'site' => "Equipment Lifecycle Management", 'history' => "Submit", 'process' => 'Equipment Lifecycle Management', 'comment' => $request->comments, 'user' => Auth::user()->name],
            ...
            if ($equipment->stage == 2) {
                ...
                //             Mail::send(
                //                 'mail.view-mail',
                //                 ['data' => $equipment, 'site' => "Equipment Lifecycle Management", 'history' => "Supervisor Approva", 'process' => 'Equipment Lifecycle Management', 'comment' => $request->comments, 'user' => Auth::user()->name],
                //                 function ($message) use ($email, $equipment) {
                //                     $message->to($email)
            ...
            if ($equipment->stage == 3) {
                ...
                //              Mail::send(
                //                  'mail.view-mail',
                //                  ['data' => $equipment, 'site' => "Equipment Lifecycle Management", 'history' => "Complete Qualification", 'process' => 'Equipment Lifecycle Management', 'comment' => $request->comments, 'user' => Auth::user()->name],
                //                  function ($message) use ($email, $equipment) {
                //                      $message->to($email)
            ...
            if ($equipment->stage == 4) {
                ...
                //             Mail::send(
                //                 'mail.view-mail',
                //                 ['data' => $equipment, 'site' => "Equipment Lifecycle Management", 'history' => "Complete Training", 'process' => 'Equipment Lifecycle Management', 'comment' => $request->comments, 'user' => Auth::user()->name],
                //                 function ($message) use ($email, $equipment) {
                //                     $message->to($email)
            ...
            if ($equipment->stage == 5) {                
                ...
                //              Mail::send(
                //                  'mail.view-mail',
                //                  ['data' => $equipment, 'site' => "Equipment Lifecycle Management", 'history' => "QA Approval", 'process' => 'Equipment Lifecycle Management', 'comment' => $request->comments, 'user' => Auth::user()->name],
                //                  function ($message) use ($email, $equipment) {
                //                      $message->to($email)
            ...
            if ($equipment->stage == 6) {
                ...
                //               Mail::send(
                //                   'mail.view-mail',
                //                   ['data' => $equipment, 'site' => "Equipment Lifecycle Management", 'history' => "Take Out of Service", 'process' => 'Equipment Lifecycle Management', 'comment' => $request->comments, 'user' => Auth::user()->name],
                //                   function ($message) use ($email, $equipment) {
                //                       $message->to($email)
...
public function RejectEquipment(Request $request, $id)
    ...
    public function equipmentChild(Request $request, $id)
    {
        ...
        $parent_type = "Equipment Lifecycle Management";
        $parent_name = "Equipment Lifecycle Management";
        ...
        elseif ($request->child_type == "Preventive Maintenance"){
            ...
            return view('frontend.preventive-maintenance.preventive_maintenance', compact('parent_id', 'record', 'parent_type', 'record_number', 'currentDate', 'formattedDate', 'due_date', 'parent_record', 'parent_record', 'parent_division_id', 'parent_initiator_id', 'parent_intiation_date', 'parent_short_description'));
        }
        else {
           
           return view('frontend.Callibration-Details.Callibration_Details_Create', compact('formattedDate', 'due_date', 'record_number', 'parent_id', 'parent_type', 'parent_record','parent_intiation_date', 'parent_initiator_id', 'parent_division_id') );
        }
    }


                // CSV Export Function for Action Item Log
  public function exportCsvequip(Request $request)
  {
    ...
    $EquipmentLc = $query->get();
      ...
      $fileName = 'equipmentlifecycle_Log.csv';
      $headers = [
      ...
      $callback = function () use ($EquipmentLc, $columns) {
          $file = fopen('php://output', 'w');
          fputcsv($file, $columns);

          if ($EquipmentLc->isEmpty()) {
              fputcsv($file, ['No records found']);
          } else {
              foreach ($EquipmentLc as $index => $row) {
                  $data = [
                      $index + 1, // Sr. No.
  ...
  public function exportExcelequip(Request $request)
  {
   $query = EquipmentLifecycleManagement::query();
   ...
   $EquipmentLc = $query->get();
      ...
      $fileName = "equipmentlifecycle_Log.xls";
      $headers = [
