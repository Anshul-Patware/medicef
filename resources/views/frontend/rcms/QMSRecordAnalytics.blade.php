            ...
            <div class="form-row">
                <div class="form-group col-md-3">
                    ...
                    <select id="process" name="process" class="form-control custom-select">
                        ...
                        <option value="Equipment Lifecycle Management" {{ request('process') == 'Equipment Lifecycle Management' ? 'selected' : '' }}>Equipment Lifecycle Management</option>
                    ...
                    <tr id="recordTable_{{ $record->id }}" class="sub-table-row" style="display:none;">
                        <td colspan="6">
                            <table class="table table-bordered">
                                <thead>
                                    <tr style="background-color: #eca035; color: #fff;">
                                        <th class="table-header">Record Stages</th>
                                        <th class="table-header">Persone Responsible</th>
                                        ...
                                        <tr class="
                                            ...
                                            <td>
                                                ...
                                                @if(trim($stageDuration['stage']) === $record->active_stage)
                                                    <i class="fa fa-car" style="color: #eca035; margin-left: 10px; animation: moveCar 1.5s ease-in-out infinite;" aria-hidden="true"></i>
                                                @endif
                                            </td>
                                            <td>
                                                {{ $stageDuration['responsible'] !== null ? $stageDuration['responsible'] : 'Not Yet Performed' }}
                                            </td>
                                            <td>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="3" style="text-align: right; font-weight: bold;">Maximum Days Spent in Opened Stage : </td>
                                    ...
                                    <tr>
                                        <td colspan="3" style="text-align: right; font-weight: bold;">Maximum Days Taken By : </td>
                                        <td style="text-align: center; font-weight: bold;">
                                            @php
                                                $maxDurationByPerson = collect($record->stage_durations)
                                                    ->filter(function ($stage) {
                                                        return $stage['days'] !== null;
                                                    })
                                                    ->groupBy('responsible')
                                                    ->map(function ($stages, $responsible) {
                                                        return $stages->max('days');
                                                    })
                                                    ->sortDesc()
                                                    ->first();
                            
                                                $maxResponsiblePerson = collect($record->stage_durations)
                                                    ->filter(function ($stage) use ($maxDurationByPerson) {
                                                        return $stage['days'] === $maxDurationByPerson;
                                                    })
                                                    ->pluck('responsible')
                                                    ->first();
                                            @endphp
                            
                                            @if($maxResponsiblePerson)
                                                {{ $maxResponsiblePerson }} ({{ $maxDurationByPerson + 1 }} days)
                                            @else
                                                N/A
                                            @endif
                                        </td>
