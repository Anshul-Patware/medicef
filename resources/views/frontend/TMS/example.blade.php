...
function displaySummary(marks) {
  // Calculate total marks as a percentage of total questions
  var percentageScore = ((marks / quizData.length) * 100).toFixed(2);

  // Check if the percentage is 80% or higher
  if (percentageScore >= 80) {
    completeBtn.style.display = 'block'; // Show the "Submit Signature" button
  } else {
    completeBtn.style.display = 'none'; // Hide the button for scores below 80%
  }
...
function submitQuiz() {
  ...
  for (var i = 0; i < quizData.length; i++) {
    ...
    if (quizData[i].type === 'Single Selection Questions') {
      if (quizData[i].choices[userAnswer[0]] === correctAnswer) { // Compare based on the value instead of index
        marks++; // Increase marks for the correct answer
      }
    } 
    else if (quizData[i].type === 'Multi Selection Questions') {
      var userSelectedChoices = userAnswer.map(index => quizData[i].choices[index]); // Convert indexes to values
      if (arraysEqual(correctAnswer, userSelectedChoices)) {
        marks++; // Increase marks for correct multi-selection answer
      }
    }
  }

  displaySummary(marks); // Show summary with the calculated marks
}

// Helper function to compare arrays (for multi-selection questions)
