...
<head>
    ...
    <style>
        * {
        ...
        body {
            background-image: linear-gradient(120deg, #f8b195 0%, #ba6715 100%);
            margin: 0;
            padding: 0;
            width: 100vw;
            height: 100vh;
        ...
        #rcms_login_block input[type="submit"] {
            ...
            width: 100%;
            padding: 9px;
            background: linear-gradient(90deg, #d58617 0%, #9e5004 100%);
            color: white;
            text-transform: uppercase;
            font-weight: bold;
            border: none;
            ...
            cursor: pointer;
            /* box-shadow: 0 4px 10px  #ba6715; */
        }

        #rcms_login_block input[type="submit"]:hover {
            background: linear-gradient(90deg, #ab680a 0%, #9e5004 100%);
            /* box-shadow: 0 6px 15px rgba(242, 185, 110, 0.6); */
            /* letter-spacing: 1px; */
        }
    ...
    <div id="rcms_login_block">
        <div class="login-form-block">
            ...
            <div class="logo-container">
                <!-- First Logo -->
                <img src="{{ asset('user/images/medicef-logo.png') }}" 
                alt="Medicef Pharma Logo" 
                class="logo-img" 
                style="scale: 0.6; margin-top: 10px;">
           
                <!-- Second Logo -->
                <img src="{{ asset('user/images/vidhyagxp.png') }}" alt="Logo 2" class="logo-img" style="scale: 0.8">
            </div>
            <!-- Welcome Text -->
...
<script>
        ...
        loginBtn.addEventListener('click', function () {
            ...
            // Retrieve the existing log data or initialize an empty array
            let logData = JSON.parse(localStorage.getItem('logData')) || [];
            logData.push(loginEntry); // Add the new login entry

            localStorage.setItem('logData', JSON.stringify(logData)); // Save updated log data to localStorage
            localStorage.setItem('currentLoginTime', loginTime); // Store the current login time for timer tracking

            // Display the login time in the UI
            loginTimeDisplay.innerText = `Login Time: ${loginTime.toLocaleTimeString()}`;

            // Enable the Logout button and disable the Login button
            document.getElementById('logoutBtn').disabled = false;
            loginBtn.disabled = true;

            // Start the header timer display
            startHeaderTimer();
        });
    </script>
