                    ...
                    <div style="padding: 5px;" class="scope-bar d-flex justify-content-end py-1">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('export.calibrationmanagement.csv')}}">
                                    <i class="fas fa-file-csv"></i> CSV Export
                                </a>
                                <a class="dropdown-item" href="{{ route('export.calibrationmanagement.excel')}}">
                                ...
                                <div class="mt-1 mb-2 bg-white" style="height: auto; padding: 10px; margin: 5px;">
                                    <div class="d-flex align-items-center">
                                        <div style="    display: grid;    grid-template-columns: 1fr 1fr 1fr 1fr 1fr;" class="filter-bar row">
                                            <!-- Department Filter -->
                                            <div class=" mb-3">
                                                <label for="initiator_group">Callibration Frequency:</label>
                                            ...
                                            <div class=" mb-3">
                                                ...
                                                <select class="custom-select" id="division_id_cm">
                                            ...
                                            <div class=" mb-3">
                                                ...
                                                <input type="date" class="custom-select" id="date_fromcm">
                                            </div>
                    
                                            <!-- End Date Filter -->
                                            <div class=" mb-3">
                                                <label for="date_to_deviation">End Date:</label>
                                                <input type="date" class="custom-select" id="date_tocm">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <table class="table table-bordered table-hover" style="background-color: #fff; border-collapse: collapse; text-align: left;">
                                    <thead style="background-color: #797979; color: #000;">
                                        <tr>
                                            ...
                                            <th class="sortable" style="background-color: #eca035" onclick="sortTable('Initiator_Group')">Callibration Frequency</th>
                                    ...
                                    <tbody id="tableData">
                                        @include('frontend.forms.Logs.filterData.calibrationmanament_data')
                                    </tbody>
                                </table>
                                <!-- Loader -->
...
<script>
    ...
    async function sortTable(column) {
        ...
        try {
            const postUrl = "{{ route('api.calibrationmanagment.filter') }}";
...
<script>
    $(document).ready(function(){
        const filterData = {
            department_calibrationmanagement: [],
            division_id_calibrationmanagement: null,
            period_calibrationmanagement: null,
            initiator_id_calibrationmanagement: [],
            date_fromcm: null,
            date_tocm: null,
            sort_column: '',
            sort_order: ''
        ...
        $('#initiator_group').change(function() {
            filterData.department_calibrationmanagement = this.value;
            filterRecords();
        });
    
        $('#division_id_cm').change('change', function() {
            filterData.division_id_calibrationmanagement = this.value;
            filterRecords();
        });
    
        document.getElementById('initiator_id').addEventListener('change', function() {
            filterData.initiator_id_calibrationmanagement = this.value;
            filterRecords();
        });
    
        document.getElementById('date_fromcm').addEventListener('change', function() {
            filterData.date_fromcm = this.value;
            filterRecords();
        });
    
        document.getElementById('date_tocm').addEventListener('change', function() {
            filterData.date_tocm = this.value;
            filterRecords();
        });
    // ===============local=============
...
<script>
    ...
    const changerelateTo = document.getElementById('division_id_cm').value;
    const Initiator = document.getElementById('initiator_id').value;
    const dateFrom = document.getElementById('date_fromcm').value;
    const dateTo = document.getElementById('date_tocm').value;
   
    const url = `/api/calibration-managementLog?department=${department}changerelateTo=${changerelateTo}&Initiator=${Initiator}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
