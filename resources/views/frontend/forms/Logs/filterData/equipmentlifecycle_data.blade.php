...
<td>{{ $Equipment->division ? $Equipment->division->name:'-'}}/ELM/{{ date('Y') }}/{{ str_pad($Equipment->record, 4, '0', STR_PAD_LEFT) }}</td>
...
<td>{{ $Equipment->due_date ? \Carbon\Carbon::parse($Equipment->due_date)->format('d-M-Y') : 'Not Applicable' }}</td>
