...
@forelse ($supplier as $index => $doc)  
        ...
        <tr>
                ...
                <td>
                    <a href="{{ url('rcms/supplier-show', $doc->id) }}" target="_blank" style="color: rgb(236,160,53);font-weight: bold; font-size: 13px;" onmouseover="this.style.color='orange'" onmouseout="this.style.color='orange'">
               {{ $doc->division ? $doc->division->name : ' Not Applicable ' }}/Supplier/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT) }}</a></td>
                <td>{{ $doc->division ? $doc->division->name : ' Not Applicable ' }}</td>
                <td>{{ Helpers::getFullDepartmentName($doc->initiation_group) ?: ' Not Applicable ' }}</td>
