                    ...
                    <div style="padding: 5px;" class="scope-bar d-flex justify-content-end py-1">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('export.auditprogram.csv')}}">
                                    <i class="fas fa-file-csv"></i> CSV Export
                                </a>
                                <a class="dropdown-item" href="{{ route('export.auditprogram.excel')}}">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div style="    display: grid;    grid-template-columns: 1fr 1fr 1fr 1fr 1fr;" class="filter-bar row">
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <select class="custom-select" id="division_id_auditprogram">
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <input type="date" class="custom-select" id="date_fromauditprogram">
                                                </div>
                        
                                                <!-- End Date Filter -->
                                                <div class=" mb-3">
                                                    <label for="date_to_deviation">End Date:</label>
                                                    <input type="date" class="custom-select" id="date_toauditprogram">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    async function sortTable(column) {
        ...
        try {
            const postUrl = "{{ route('api.auditprogram.filter') }}";
...
<script>
    $(document).ready(function(){
        const filterData = {
            department_auditprogram: [],
            division_id_auditprogram: null,
            initiator_id_auditprogram: [],
            date_fromauditprogram: null,
            date_toauditprogram: null,
            sort_column: '',
            sort_order: ''
        };
    
        $('#initiator_group').change(function() {
            filterData.department_auditprogram = this.value;
            filterRecords();
        });
    
        $('#division_id_auditprogram').change('change', function() {
            filterData.division_id_auditprogram = this.value;
            filterRecords();
        });
    
        document.getElementById('initiator_id').addEventListener('change', function() {
            filterData.initiator_id_auditprogram = this.value;
            filterRecords();
        });
    
        document.getElementById('date_fromauditprogram').addEventListener('change', function() {
            filterData.date_fromauditprogram = this.value;
            filterRecords();
        });
    
        document.getElementById('date_toauditprogram').addEventListener('change', function() {
            filterData.date_toauditprogram = this.value;
            filterRecords();
        });
    // ===============local=============
...
<script>
    ...
    const changerelateTo = document.getElementById('division_id_auditprogram').value;
    const Initiator = document.getElementById('initiator_id').value;
    const dateFrom = document.getElementById('date_fromauditprogram').value;
    const dateTo = document.getElementById('date_toauditprogram').value;
    const url = `/api/audit-programpdf?department=${department}&changerelateTo=${changerelateTo}&Initiator=${Initiator}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
