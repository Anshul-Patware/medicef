...
@forelse ($cm as $index => $doc)  
        ...
        <tr>
                ...
                <td>
                    <a href="{{ route('showCalibrationDetails', $doc->id) }}" style="color:  rgb(236,160,53);" onmouseover="this.style.color='orange'" onmouseout="this.style.color='orange'">
               {{ $doc->division ? $doc->division->name : ' Not Applicable ' }}/CM/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT) }}</a></td>
                <td>{{ $doc->division ? $doc->division->name : ' Not Applicable ' }}</td>
                <td>{{ $doc->callibration_frequency ?: ' Not Applicable ' }}</td>
