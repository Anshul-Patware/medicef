...
@forelse ($marketcomplaint as $index => $doc)  
    ...
    <tr>
        ...
        <td>
            <a href="{{ route('marketcomplaint.marketcomplaint_view', $doc->id) }}" target="_blank" style="color: rgb(236,160,53); font-weight: bold; font-size: 13px;">
               {{ $doc->division ? $doc->division->name : ' Not Applicable ' }}/CM/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT) }}
            </a>
        </td>
        ...
        <td>{{ $doc->due_date_gi ? \Carbon\Carbon::parse($doc->due_date_gi)->format('d-M-Y') : ' Not Applicable ' }}</td>
