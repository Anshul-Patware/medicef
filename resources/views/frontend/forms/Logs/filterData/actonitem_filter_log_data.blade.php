...
@forelse($actionitem as $action_item) 
        ...
        <tr>
                <td>{{$loop->index + 1}}</td>
                <td>{{ $action_item->intiation_date ? \Carbon\Carbon::parse($action_item->intiation_date)->format('d-M-Y') : 'Not Applicable' }}</td>

                <td><a href="{{ route('actionItem.show', $action_item->id) }}" style="color: #eca035;font-size: 13px;" onmouseover="this.style.color='orange'" onmouseout="this.style.color='orange'">{{$action_item->division ? $action_item->division->name : ' Not Applicable '}}/AI/{{ date('Y') }}/{{ str_pad($action_item->record, 4, '0', STR_PAD_LEFT)}}</a></td>
                ...
                <td>{{ $action_item->due_date ? \Carbon\Carbon::parse($action_item->due_date)->format('d-M-Y') : 'Not Applicable' }}</td>
