                    ...
                    <div style="padding: 5px;" class="scope-bar d-flex justify-content-end py-1">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('export.csv.extension') }}">
                                    <i class="fas fa-file-csv"></i> CSV Export
                                </a>
                                <a class="dropdown-item" href="{{ route('export.excel.extension') }}">
                                    ...
                                    <div class="mt-1 mb-2 bg-white" style="height: auto; padding: 10px; margin: 5px;">
                                        <div class="d-flex align-items-center">
                                            <div style="    display: grid;    grid-template-columns: 1fr 1fr 1fr 1fr 1fr;" class="filter-bar row">
                                                <!-- Department Filter -->
                                                <div class=" mb-3">
                                                    <label for="initiator_group">Priority:</label>
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <input type="date" class="custom-select" id="date_from_extension">
                                                </div>
                        
                                                <!-- End Date Filter -->
                                                <div class=" mb-3">
                                                    <label for="date_to_deviation">End Date:</label>
                                                    <input type="date" class="custom-select" id="date_to_extension">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <table class="table table-bordered table-hover" style="background-color: #fff; border-collapse: collapse; text-align: left;">
                                    <thead style="background-color: #797979; color: #000;">
                                        <tr>
                                            ...
                                            <th class="sortable" style="background-color: #eca035" onclick="sortTable('Initiator_Group')">Priority</th>
                                    ...
                                    <tbody id="tableData">
                                    @include('frontend.forms.Logs.extension_data')   </tbody>
                                </table>
                                <!-- Loader -->
...
<script>
    $(document).ready(function(){
        const filterData = {
            ...
            division_id_extension: null,
            date_from_extension: null,
        ...
        $('#initiator_group').change(function() {
            filterData.department_extension = this.value;
            filterRecords();
        });
    
        $('#division').change('change', function() {
            filterData.division_id_extension = this.value;
            filterRecords();
        });
    
     
        document.getElementById('date_from_extension').addEventListener('change', function() {
            filterData.date_from_extension = this.value;
            filterRecords();
        });
    
        document.getElementById('date_to_extension').addEventListener('change', function() {
            filterData.date_to_extension = this.value;
            filterRecords();
        });
    // ===============local=============
...
<script>
    ...
    const changerelateTo = document.getElementById('division').value;
    ...
    const url = `/api/print-extension?department=${department}&changerelateTo=${changerelateTo}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
