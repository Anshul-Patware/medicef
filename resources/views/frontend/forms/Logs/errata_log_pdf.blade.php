...
<body>
            ...
            <tr>
                <td class="w-50">
                    <div class="header-title">
                        <h3 style="margin-left: 100px">Errata Log Report</h3>
                    </div>
                </td>
    ...
    <table>
        <thead>
           <th class="sortable"  onclick="sortTable('id')">Sr. No.</th>
                                            ...
                                            <th class="sortable"  onclick="sortTable('Initiator_Group')">Department</th>
    ...
    <table>
            <tbody>
                ...
                <tr>
                <td>{{$loop->index+1}}</td>
                                            <td>{{ $doc->intiation_date ? \Carbon\Carbon::parse($doc->intiation_date)->format('d-M-Y') : 'Not Yet Data Available' }}</td>

                                            <td>{{ $doc->division_id ? $doc->division->name : 'Not Yet Data Available'  }}/Errata/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT) }}</td>
                                            <td>{{ $doc->division_id ? $doc->division->name : 'Not Yet Data Available'}}</td>
                                            <td>{{Helpers::getNewInitiatorGroupFullName($doc->department_code) ?? 'Not Yet Data Available'}}</td>
                                            <td>{{$doc->short_description ?? 'Not Yet Data Avaiable'}}</td>
                                            <td>{{ $doc->initiator_id ? $doc->initiator->name : 'Not Yet Data Available' }}</td>
                                            <td>{{$doc->due_date ? $doc->due_data : 'Not Yet Data Available'}}</td>
                                            <td>{{$doc->status}}</td>
                </tr>
                @endforeach
            </tbody>
