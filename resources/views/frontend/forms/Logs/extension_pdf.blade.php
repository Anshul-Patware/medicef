...
<body>
            ...
            <tr>
                <td class="w-50">
                    <div class="header-title">
                        <h3 style="margin-left: 100px">Due Date Extension Log Report</h3>
                    </div>
                </td>
    ...
    <table>
            <tbody>
                ...
                <tr>
                <td>{{$loop->index + 1}}</td>
                <td>{{$doc->initiation_date ? Carbon::parse($doc->initiation_date)->format('d-M-Y') : ' Not Applicable '}}</td>
                <td>{{$doc->division ? $doc->division->name : ' Not Applicable '}}/Ex/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT)}}</td>
                <td>{{$doc->division ? $doc->division->name : ' Not Applicable '}}</td>
                <td>{{ $doc->priority_data ? $doc->priority_data : ' Not Applicable ' }}</td>
                <td>{{$doc->short_description}}</td>
                <td>{{$doc->current_due_date ?  Carbon::parse($doc->current_due_date)->format('d-M-Y'): ' Not Applicable'}}</td> 
                <td>{{$doc->status ? $doc->status : ' Not Applicable '}}</td>
            </tr>
                @endforeach
            </tbody>
        </table>
